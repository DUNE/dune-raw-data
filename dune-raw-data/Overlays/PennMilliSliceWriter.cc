...
std::shared_ptr<dune::PennMicroSliceWriter>
dune::PennMilliSliceWriter::reserveMicroSlice(uint32_t ms_max_bytes)
{
...
int32_t dune::PennMilliSliceWriter::finalize(bool override, uint32_t data_size_bytes,
#ifdef PENN_DONT_REBLOCK_USLICES
					     ...
					     uint64_t end_timestamp, uint32_t width_in_ticks, uint32_t overlap_in_ticks)
{
  ...
  *(reinterpret_cast<dune::PennMilliSliceWriter::checksum_t *>(end_ptr)) = calculateChecksum();
  header_()->millislice_size += sizeof(dune::PennMilliSliceWriter::checksum_t);
#endif
