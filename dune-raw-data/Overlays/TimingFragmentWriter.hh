#ifndef dune_artdaq_Overlays_TimingFragmentWriter_hh
#define dune_artdaq_Overlays_TimingFragmentWriter_hh

////////////////////////////////////////////////////////////////////////
// TimingFragmentWriter
//
// Class derived from TimingFragment which allows writes to the data (for
...
// TimingFragment, including its reference to the artdaq::Fragment
...
namespace dune {
  class TimingFragmentWriter;
}


class dune::TimingFragmentWriter: public dune::TimingFragment {
public:


  TimingFragmentWriter(artdaq::Fragment & f); 
  ...
  // We'll need to hide the const version of header in TimingFragment in
...
// TimingFragment::Metadata object, otherwise it throws

dune::TimingFragmentWriter::TimingFragmentWriter(artdaq::Fragment& f ) :
  TimingFragment(f), artdaq_Fragment_(f) {
   
    // If this assert doesn't hold, then can't call
    // "words_to_frag_words_" below, translating between the
    // TimingFragment's standard data type size and the
    ...
    if (! artdaq_Fragment_.hasMetadata() ) {
      throw cet::exception("Error in TimingFragmentWriter: passed artdaq::Fragment object doesn't contain metadata");
    }

    if (artdaq_Fragment_.dataSizeBytes() > 0) {
      throw cet::exception("Error in TimingFragmentWriter: passed artdaq::Fragment object already contains payload");
    }
...
inline dune::TimingFragment::adc_t * dune::TimingFragmentWriter::dataBegin() {
  assert(artdaq_Fragment_.dataSizeBytes() > sizeof(Header) );
  return reinterpret_cast<adc_t *>(header_() + 1);
}

inline dune::TimingFragment::adc_t * dune::TimingFragmentWriter::dataEnd() {
  return dataBegin() + total_adc_values();
}


inline void dune::TimingFragmentWriter::resize(size_t nAdcs) {
  auto es(calc_event_size_words_(nAdcs));
  artdaq_Fragment_.resizeBytes( sizeof(Header::data_t) * es );
  header_()->event_size = es;
}

inline size_t dune::TimingFragmentWriter::calc_event_size_words_(size_t nAdcs) {
  return adcs_to_words_(nAdcs) + hdr_size_words();
}

inline size_t dune::TimingFragmentWriter::adcs_to_words_(size_t nAdcs) {
...
#endif /* dune_artdaq_Overlays_TimingFragmentWriter_hh */
