...
void MakePdspChannelMap_v4() {
  ...
  ofstream fmap;
  fmap.open ("protoDUNETPCChannelMap_v4.txt");
  int nCrate = 6;
  int felixCrate = 5;   // index of the FELIX crate -- the last one.
  int nSlotPerCrate = 5;
  int nFiberPerSlot = 4;
  int nChipPerFEMB = 8;
  int nASICPerChip = 2;
     	 ...
     	 for(int fiberNo = 0;fiberNo<nFiberPerSlot;fiberNo++) {
     	 	 ...
     	 	 for(int chipNo = 0;chipNo<nChipPerFEMB;chipNo++) {  
     	 	 	 ...
     	 	 	 for(int asicNo = 0;asicNo<nASICPerChip;asicNo++) {
					 ...
					 if (crateNo != felixCrate)
					   {
     	 	 	 	 	     StreamChannel = rceCh[chipNo][chipChannel]; 
					   }
					 else
					   {
     	 	 	 	 	     StreamChannel = felixCh[chipNo][chipChannel]; 
					   }
