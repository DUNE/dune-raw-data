  ...
  while(pl_ptr < (buffer_ + pl_size)) {
    ...
    switch (type) {
      case 0x1: // counter word
        mf::LogInfo("PennMicroSlice") << "Sample type: counter : [" << std::bitset<3>(type) << "]" << std::endl;
        break;
      case 0x2: // trigger word
        mf::LogInfo("PennMicroSlice") << "Sample type: trigger : [" << std::bitset<3>(type) << "]" << std::endl;
        break;
      case 0x4: // Checksum
        mf::LogInfo("PennMicroSlice") << "Sample type: checksum : [" << std::bitset<3>(type) << "]" << std::endl;

      case 0x7: // timestamp word
        mf::LogInfo("PennMicroSlice") << "Sample type: timestamp : [" << std::bitset<3>(type) << "]" << std::endl;
        break;
      case 0x0: //self test
        mf::LogInfo("PennMicroSlice") << "Sample type: self test : [" << std::bitset<3>(type) << "]" << std::endl;
        break;

      default:
        mf::LogError("PennMicroSlice") << "Unexpected header...";
    }
